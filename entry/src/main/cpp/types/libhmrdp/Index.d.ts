...
/** 返回鸿蒙桌面热键：mods 位掩码 1=Ctrl 2=Alt 4=Shift，keyCode 为 OHOS 键码（KeyCode 枚举值）。
 *  命中的按键被本地消费、不转发远端，并回调 onTrigger（用于最小化回桌面）。
 *  mods 或 keyCode 传 0 = 关闭。 */
export const setDesktopHotkey: (mods: number, keyCode: number, onTrigger?: () => void) => void;
...
/** 当前单指按住状态，供 ArkTS 画长按进度圈 */
export interface TouchHold {
  /** 按住且未明显漂移的毫秒数；< 0 表示当前没有这样的按住 */
  ms: number;
  /** 按下点（surface 像素） */
  x: number;
  y: number;
...
export const getTouchHold: () => TouchHold;
